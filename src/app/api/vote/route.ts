import { ANSWERS_COLLECTION, db, QUESTION_COLLECTION, VOTE_COLLECTION } from "@/models/name";
...
export async function POST(request: NextRequest) {
    try {
        ...
        const response = await databases.listDocuments(db, VOTE_COLLECTION, [
        ...
        if (response.documents.length > 0) {
            await databases.deleteDocument(db, VOTE_COLLECTION, response.documents[0].$id);

            // Decrease the reputation of the question/answer author
            const questionOrAnswer = await databases.getDocument(
                db,
                type === "question" ? QUESTION_COLLECTION : ANSWERS_COLLECTION,
                typeId
            );
        ...
        if (response.documents[0]?.voteStatus !== voteStatus) {
            const doc = await databases.createDocument(db, VOTE_COLLECTION, ID.unique(), {
                type,
                typeId,
                voteStatus,
                votedById,
